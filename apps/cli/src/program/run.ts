...
import { CliError, EXIT_CODES, PROGRAM, UsageError, type ExitCode, type Io } from "../shared/index.ts";
import { USAGE } from "./constants.ts";
import { parseCommand } from "./parse-command.ts";
import type { Command, Topic } from "./types.ts";

const HELP: Readonly<Record<NonNullable<Topic>, string>> = { check: CHECK_HELP, catalog: CATALOG_HELP };
...
export async function run(argv: readonly string[], io: Io): Promise<ExitCode> {
  ...
  try {
    ...
    if (command?.name === "check" && command.json) io.stdout(JSON.stringify({ ok: false, error: error.message }));
    else io.stderr(`${PROGRAM}: ${error.message}${error instanceof UsageError ? `; run ${PROGRAM} --help` : ""}`);
