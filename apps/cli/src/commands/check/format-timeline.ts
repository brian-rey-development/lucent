...
export function formatTimeline({ scenes }: Timeline, limit: number): readonly string[] {
  ...
  const more = scenes.length > limit ? [`+${scenes.length - limit} more scenes; use --scene or --json`] : [];
...
function formatScene({ id, start, duration, cues }: SceneTimeline, width: number): string {
  ...
  const marks = cues.slice(0, MAX_CUES).map(({ phrase, time }) => `${quote(phrase)} +${time.toFixed(1)}s`);
