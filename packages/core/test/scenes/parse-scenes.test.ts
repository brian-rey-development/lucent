...
describe("parseScenes", () => {
  ...
  it("accepts ~~~ fences and an info string after the language", () => {
    expect(scene(`## a\n\nText.\n\n~~~scene\ndo: []\n~~~`).block).toEqual({ text: "do: []", firstLine: 6 });
    expect(scene(`## a\n\nText.\n\n\`\`\`scene title\ndo: []\n\`\`\``).block).toEqual({ text: "do: []", firstLine: 6 });
  });
  ...
  it.each([
    ...
    ["### plasma", 'E124 1:1 document "### plasma" is not a scene heading; fix: use ## plasma or remove it'],
  ...
  it.each([
    [`## a\n\nText.\n\n${BLOCK}\n\n${BLOCK}`, "E126 10:1 a duplicate scene block; fix: merge both into one"],
    ...
    [`## a\n\nText.\n\n${BLOCK}\n\n## a\n\nText.\n\n${BLOCK}`, "E122 10:1 a duplicate scene id a; fix: rename one"],
    [`## a\n\nText.\n<!-- open\n\n${BLOCK}`, "E130 4:1 a comment never closed; fix: close it with -->"],
    [
      `## a\n\n> es: Hola.\n\nText.\n\n${BLOCK}`,
      "E131 3:1 a translation without a paragraph; fix: put it right after its paragraph",
    ],
    [`## a\n\nText.\n> a quote\n\n${BLOCK}`, "E132 4:1 a malformed translation; fix: write > xx: TEXT"],
    [`## a\n\n> a quote\n\nText.\n\n${BLOCK}`, "E132 3:1 a malformed translation; fix: write > xx: TEXT"],
    [`## a\n\nText.\n> ES: Hola.\n\n${BLOCK}`, "E132 4:1 a uppercase language ES; fix: write > es: TEXT"],
    [`## a\n\nText.\n\n(pause 1 s)\n\n${BLOCK}`, "E138 5:1 a malformed pause; fix: write (pause 1.5s)"],
    [`## a\n\nText.\n\n(Pause 90s)\n\n${BLOCK}`, "E138 5:1 a pause of 90s; fix: use more than 0s and at most 60s"],
    [`## a\n\nText.\n\n(pause 0s)\n\n${BLOCK}`, "E138 5:1 a pause of 0s; fix: use more than 0s and at most 60s"],
  ])("reports %j", (text, expected) => {
    expect(problems(text)).toEqual([expected]);
  });

  it("reports a file without scenes once", () => {
    expect(problems("")).toEqual(["E120 1:1 document no scenes; fix: add ## scene-id and its narration"]);
    expect(problems("Stray.")).toEqual(["E123 1:1 document text outside a scene; fix: add ## scene-id above it"]);
  });

  it("starts a new paragraph after a translation", () => {
    const { narration } = scene(`## a\n\nOne.\n> es: Uno.\nTwo.\n> es: Dos.\n\n${BLOCK}`);

    expect(narration.map((item) => (item.kind === "paragraph" ? item.text : item.kind))).toEqual(["One.", "Two."]);
  });
...
describe("paragraphs", () => {
  it("parses cues with ids, positions and word indexes", () => {
    const { cues, text, lines } = paragraph(`## a\n\nThese are [red cells|rbc] and [white\ncells].\n\n${BLOCK}`);
    ...
    expect(lines.map(({ spoken }) => spoken)).toEqual(["These are  red cells      and  white", "cells ."]);
  });
  ...
  it("keeps the words of a malformed cue", () => {
    ...
    expect(cues).toEqual([expect.objectContaining({ phrase: "red cells", id: undefined, malformed: true })]);
  });
});
