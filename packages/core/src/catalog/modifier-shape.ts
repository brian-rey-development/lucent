...
export function modifierShape(level: ModifierLevel, element: ElementKind): Readonly<Record<string, z.ZodOptional>> {
  const allowed = MODIFIERS_BY_LEVEL[level].filter((name) => element !== "none" || !ELEMENT_MODIFIERS.has(name));
