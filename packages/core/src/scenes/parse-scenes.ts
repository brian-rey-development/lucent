...
export function parseScenes(lines: readonly string[], firstLine: number): Result<readonly SceneSource[]> {
  const built = buildScenes(tokenize(lines, firstLine));
  if (built.value.length > 0 || built.diagnostics.length > 0) return built;
  return {
    value: [],
    diagnostics: [createDiagnostic(NO_SCENES, { where: DOCUMENT, position: { line: firstLine, column: 1 } })],
  };
}
