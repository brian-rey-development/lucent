...
import { paragraphsOf, type Cue, type Scene, type Step, type Video } from "../model/index.ts";
...
function analyzeScene(scene: Scene): readonly Diagnostic[] {
  ...
  const resolutions = (steps ?? []).map((step, position) =>
    resolveStepCue(scene, index, step, position === 0),
  );
  const withDigits = cues.filter(({ phrase }) => DIGIT.test(phrase));
  const excluded = new Set([...index.duplicates, ...withDigits]);
  const unused = allResolved(steps, resolutions) ? unusedCues(cues, resolutions, excluded) : [];
  return [
...
function allResolved(
  ...
  if (steps === undefined || steps.some(({ at }) => at.kind === "invalid")) return false;
  return !resolutions.some(({ used, diagnostics }) => used === undefined && diagnostics.length > 0);
}

function unusedCues(
  cues: readonly Cue[],
  resolutions: readonly Resolution[],
  excluded: ReadonlySet<Cue>,
): readonly Cue[] {
  const used = new Set(resolutions.flatMap(({ used: cue }) => cue ?? []));
  return cues.filter((cue) => !cue.malformed && !used.has(cue) && !excluded.has(cue));
}

function report(
  scene: Scene,
  cues: readonly Cue[],
  problemOf: (cue: Cue) => Problem,
): readonly Diagnostic[] {
  return cues.map((cue) =>
