...
export function keepElements(
  kept: readonly KeptElement[] | undefined,
  previous: Screen,
  scene: Scene,
): Result<Screen> {
...
function notKept(
  ...
  const fix =
    closest === undefined ? "show it in the previous scene or drop it" : `keep $${closest}`;
  const problem =
    reason === undefined
      ? keptNotOnScreen(id, scene.id, fix)
      : { ...gone(id, reason), code: "E208" as const };
