...
const SYNTAX: Readonly<Record<string, Text>> = {
  ...
  MULTILINE_IMPLICIT_KEY: { message: "key spans two lines", fix: "put the key and its : on one line" },
...
const TOKEN_TEXT: Readonly<Record<"alias" | "anchor" | "tag" | "directive" | "duplicate", (token: string) => Text>> = {
  alias: (token) => ({ message: `${quote(token)} reads as a YAML alias`, fix: "quote the value" }),
  anchor: (token) => ({ message: `${quote(token)} reads as a YAML anchor`, fix: "quote the value" }),
...
const BY_MESSAGE: readonly (readonly [RegExp, Text])[] = [
  ...
  [/end with a \}$/, { message: "unclosed {", fix: "add }" }],
  [/without - indicator$/, { message: "list item without -", fix: "add - or indent it under the item above" }],
];
...
function syntaxText(code: string, message: string): Text {
  ...
  return known?.[1] ?? SYNTAX[code] ?? { message: "invalid YAML", fix: "quote the value or fix the indentation" };
}

export function unknownKey(key: string, closest: string | undefined): Problem {
  return { code: "E104", message: `unknown key ${key}`, fix: closest === undefined ? "remove it" : `use ${closest}` };
}
