...
import { createDiagnostic, formatWhere, type Diagnostic, type Result } from "../diagnostics/index.ts";
import { DEFAULT_MESSAGE } from "./constants.ts";
import { describeIssue } from "./describe-issue.ts";
import type { Issue, Site } from "./types.ts";

export function validateValue<T>(schema: z.ZodType<T>, value: unknown, site: Site): Result<T | undefined> {
...
function toDiagnostic({ problem, path, at }: Issue, { locate, path: base, place }: Site): Diagnostic {
